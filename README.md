...
Интеграция платёжного шлюза [Platega](https://platega.io) с XenForo 1.5, 2.2 и 2.3. В XenForo 2.x поддерживает оплату повышений пользователей и покупок в дополнениях через стандартные платёжные профили. Для 1.5 доступен отдельный модуль повышений.
...
| Версия форума | Архив | Установка |
| --- | --- | --- |
| XenForo 2.2 / 2.3 | Evrik-Platega-1010070.zip | [Через панель дополнений](docs/INSTALL.md) |
| XenForo 1.5 | Evrik-Platega-XF1-1000070.zip | [Загрузка файлов и импорт XML](legacy/README.md) |

Пакет для 1.5 поддерживает стандартные повышения пользователей. Возможности ниже относятся к основному модулю для 2.x.
...
| --- | --- |
| XenForo | Основной пакет: 2.2 / 2.3; отдельный пакет: 1.5. Проверенные выпуски — в таблице совместимости |
...
Перед рабочим запуском выполните контрольную оплату на своём магазине Platega. Локальные проверки используют тестовые ответы API; сквозная проверка с платёжным шлюзом перед выпуском не выполнялась.

## Быстрая установка для XenForo 2.x

1. Скачайте `Evrik-Platega-1010070.zip` из релиза. Архивы GitHub **Source code** не предназначены для установки через панель.
...
После изменения версии дополнения в `main` workflow `Release` создаёт тег и релиз (для бета-версий — предварительный): исходники проверяются, архив собирается и загружается в GitHub Releases вместе с контрольной суммой.

## Лицензия

[MIT](LICENSE) · Copyright © 2026 [Evrik (Evr1kys)](https://github.com/Evr1kys).
